...
<template>
    <div class="flex flex-row min-h-screen w-full bg-gray-100 text-gray-700" x-data="layout">
        ...
        <label class="border-transparent rounded-lg text-center px-4 py-2 mx-auto md:mx-0 my-2 bg-gray-100 font-medium z-10 shadow-lg">
          Divisions
        </label>
          <nav class="flex flex-col bg-slate-700 w-60 px-2 py-2 text-gray-900 border border-purple-900">
            <ul class="ml-1">
                  <li v-for="div in divisions" class="mb-1 px-0 py-2 text-gray-100 flex flex-row  border-gray-300 hover:text-black   hover:bg-gray-300  hover:font-bold rounded rounded-lg">
                    <span>
                    ...
                    <a href="#" @click="selectDiv(div)">
                        <span class="ml-1">{{ div.name }}</span>
                    </a>
                  </li>
            </ul>
          </nav>
        <label v-if="teams.length > 0" class="border-transparent rounded-lg text-center px-4 py-2 mx-auto md:mx-0 my-2 bg-gray-100 font-medium z-10 shadow-lg">
          {{divName}}
        </label>
            <nav v-if="teams.length > 0" class="flex flex-col bg-slate-700 w-60 px-2 py-2 text-gray-900 border border-purple-900">
              <ul class="ml-1">
